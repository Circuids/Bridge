...
public sealed class BridgeSafeAreaMauiConformanceSuite
{
    private readonly IServiceScopeFactory _serviceScopeFactory;

    public BridgeSafeAreaMauiConformanceSuite(IServiceScopeFactory serviceScopeFactory)
    {
    ...
    [PulseCase]
    public async Task InitializeAsync_reads_non_negative_real_Maui_safe_area_insets()
    {
        await using var scope = _serviceScopeFactory.CreateAsyncScope();
        var safeArea = scope.ServiceProvider.GetRequiredService<IBridgeSafeArea>();
    ...
    [PulseCase]
    public async Task InitializeAsync_raises_SafeAreaChanged_with_current_value()
    {
        await using var scope = _serviceScopeFactory.CreateAsyncScope();
        var safeArea = scope.ServiceProvider.GetRequiredService<IBridgeSafeArea>();
